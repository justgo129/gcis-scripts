...
# This creates a generic publication for the references having a reftype = 'Thesis'.
...
my $n_update = 0;
for (@$refs) {
    next if $_->{attrs}->{reftype} ne "Thesis";
    next if $_->{child_publication_id};
    $n_update++;
    last if $n_update > $max_update;

    my $generic_pub->{attrs} = {};
    for my $a (qw(reftype Author Date University Year Title DOI URL)) {
        $generic_pub->{attrs}->{$a} = $_->{attrs}->{$a};
    }

    $generic_pub->{attrs}->{'Academic Department'} = $_->{attrs}->{'Academic Department'};
    $generic_pub->{attrs}->{Pages} = $_->{attrs}->{'Number of Pages'};
...
thesis-generic - adds and associates generic child pubs with Thesiss.

=head1 SYNOPSIS

thesis-generic [options]
...
B<thesis-generic.pl> creates child publications of class 'generic' for reference types of
class 'Thesis.'  The program is designed to allow users to select
