...
# This creates a generic publication for the references having a reftype = 'Conference Paper'.
...
for (@$refs) {
    ...
    for my $a (qw(reftype Author Date Year Title DOI URL)) {
...
cproc-generic - adds and associates generic child pubs with conference papers.
...
class 'Conference Paper.'  The program is designed to allow users to select
