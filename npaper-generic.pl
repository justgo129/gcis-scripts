...
# This creates a generic publication for the references having a reftype = 'Newspaper Article'.
...
my $n_update = 0;
for (@$refs) {
    next if $_->{attrs}->{reftype} ne "Newspaper Article";
    next if $_->{child_publication_id};
    $n_update++;
    last if $n_update > $max_update;

    my $generic_pub->{attrs} = {};
    for my $a (qw(reftype Newspaper Title Pages Year URL)) {
        $generic_pub->{attrs}->{$a} = $_->{attrs}->{$a};
    }

    $generic_pub->{attrs}->{Date} = $_->{attrs}->{'Issue Date'};
    $generic_pub->{attrs}->{Author} = $_->{attrs}->{Reporter};
